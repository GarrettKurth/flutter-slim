...
# This function edits the .builder.yml file with the given flutter version for the given docker tag
edit_builder_file_for_tag() {
    builder_file=".builder.yml"
    docker_tag=$1
    version=$2

    # env for yq
    docker_tag=$docker_tag version=$version \
        yq -i '(.docker_builder.env.matrix[] | select(.DOCKER_TAG == env(docker_tag)) | .FLUTTER_VERSION) = env(version)' $builder_file
}
...
edit_builder_file_for_tag "stable" "$stable_version"
edit_builder_file_for_tag "latest" "$stable_version"
edit_builder_file_for_tag "beta" "$beta_version"
